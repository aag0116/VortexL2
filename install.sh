...
#
# Usage: curl -fsSL https://raw.githubusercontent.com/iaag0116/VortexL2/main/install.sh | sudo bash
#
...
REPO_URL="https://github.com/aag0116/VortexL2.git"
REPO_BRANCH="main"
...
if command -v git &> /dev/null; then
    git clone --depth 1 --branch "$REPO_BRANCH" "$REPO_URL" "$INSTALL_DIR" 2>/dev/null || {
        ...
        curl -fsSL "https://github.com/aag0116VortexL2/archive/refs/heads/${REPO_BRANCH}.tar.gz" | \
            tar -xz -C "$INSTALL_DIR" --strip-components=1
    }
else
    mkdir -p "$INSTALL_DIR"
    curl -fsSL "https://github.com/aag0116/VortexL2/archive/refs/heads/${REPO_BRANCH}.tar.gz" | \
        tar -xz -C "$INSTALL_DIR" --strip-components=1
fi
